...
data_PH = [7.1,7.0,7.3,7.1,7.3,7.1,7.2,7.2,7.0,7.0,6.9,7.1,7.2,7.2,7.1,7.0,7.0,7.0,7.1]
data_TE = [18.8,18.6,19.4,19.5,23.3,20.7,21.3,23.6,17.8,16.6,17.8,17.3,18.8,18.0,19.2,18.0,17.7,17.3,19.3]
data_AL = [53,50,46,40,48,50,50,53,35,40,42,40,55,50,49,37,42,41,42]
data_PAC = [1300,1300,1400,1400,900,900,900,900,1200,1100,1100,1100,900,1000,1000,1200,1200,1100,1100]
...
PH = np.linspace(6.90, 7.53, 200)
AL = np.linspace(35.0, 60.3, 200)
TE = np.linspace(16.6, 24.9, 200)



PH_small = fuzz.trimf(PH, [6.90,6.90,7.30])
PH_big = fuzz.trimf(PH, [7.25,7.25,7.50])
...
qf_tsk.add_input_fuzzysets(var_name='varPH', set_names=['small', 'big'], sets=[PH_small, PH_big])
qf_tsk.add_input_fuzzysets(var_name='varAL', set_names=['small', 'big'], sets=[AL_small, AL_big])
qf_tsk.add_input_fuzzysets(var_name='varTE', set_names=['small', 'big'], sets=[TE_small, TE_big])


i_data = 7
varPH, varAL, varTE, varTB1, varTB2 = data_PH[i_data], data_AL[i_data], data_TE[i_data], data_TB1[i_data], data_TB2[i_data]
c=consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)
print('consequents ', c)

print( 'inputs varPH, varAL, varTE, varTB1, varTB2 :', varPH, varAL, varTE, varTB1, varTB2 )
rules = ['if varPH is small and varAL is small and varTE is small then '+ str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[0]),
         'if varPH is small and varAL is small and varTE is big then ' + str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[1]),
         'if varPH is small and varAL is big and varTE is small then ' + str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[2]),
         'if varPH is small and varAL is big and varTE is big then ' + str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[3]),
         'if varPH is big and varAL is small and varTE is small then ' + str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[4]),
         'if varPH is big and varAL is small and varTE is big then ' + str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[5]),
         'if varPH is big and varAL is big and varTE is small then ' + str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[6]),
         'if varPH is big and varAL is big and varTE is big then ' + str(
             consequents(TB1=varTB1, TB2=varTB2, PH=varPH, TE=varTE, AL=varAL)[7]),

         ]

qf_tsk.set_rules(rules)

qf_tsk.build_inference_qc({'varPH':varPH, 'varAL':varAL, 'varTE':varTE}, draw_qc=True)
y_norm = qf_tsk.execute('qasm_simulator', 65536, plot_histo=False)
y_norm = qf_tsk.sign * math.sqrt(abs(y_norm))
print(y_norm)

#y = renormalize(y_norm, [min([abs(i) for i in list(qf_tsk.output_constants.values())]),max([abs(i) for i in list(qf_tsk.output_constants.values())])], [min(data_PAC), max(data_PAC)])
#print(y)
...
qf_tsk.add_input_fuzzysets(
    var_name="x",
    set_names=["x_NL", "x_NS", "x_0", "x_PS", "x_PL"],
    sets=[x_NL, x_NS, x_0, x_PS, x_PL],
)
qf_tsk.add_input_fuzzysets(
    var_name="v",
    set_names=["v_NL", "v_NS", "v_0", "v_PS", "v_PL"],
    sets=[v_NL, v_NS, v_0, v_PS, v_PL],
)
...
for _ in range(1):
    ...
    qf_tsk.build_inference_qc({"x": x_0, "v": v_0}, draw_qc=True)
